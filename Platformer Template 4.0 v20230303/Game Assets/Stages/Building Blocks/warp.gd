...
func _physics_process(delta):
	if !Engine.is_editor_hint():
		if warp_area.has_point(Global.player.global_position - global_position):
			warp()
	else:
		queue_redraw()
...
func adjust_warp_offset():
	...
	Global.game.transition_offset_pos = Global.player.global_position - global_position
	Global.game.transition_offset_pos.x *= 1-multipliery
	Global.game.transition_offset_pos.y *= multipliery
	Global.game.transition_velocity = Global.player.velocity
	Global.game.transition_jump_stamina = Global.player.jump_stamina
